...
function find_matches(boardGrid) {
    ...
    for (var i = 0; i < ROWS; ++ i) {
        for (var j = 0; j < COLS; ++ j) {
            ...
            if (cur_orb.type == "X") { continue; } //if this position isn't part of a series, skip it.
            ...
            while (stack.length) { //while at least one item on the stack...
                ...
                if (scratch_board.grid[n.row][n.col].type == "undefined" ||
                		scratch_board.grid[n.row][n.col].type != cur_orb.type) { continue; }  //Stop searching if this orb doesn't match the board.
                ++ count;

                scratch_board.grid[n.row][n.col].type = "X"; //we have n. clear the scratch board for this starting spot.
                if (n.row > 0) { stack.push(new Coordinate(n.row-1, n.col)); } //if we aren't on the first row, add the orb above this one to the stack.
                if (n.row < ROWS-1) { stack.push(new Coordinate(n.row+1, n.col)); } //if we aren't on the last row, add the orb below this one to the stack.
                if (n.col > 0) { stack.push(new Coordinate(n.row, n.col-1)); } //if we aren't in the first column, add the orb to the left of this one to the stack.
                if (n.col < COLS-1) { stack.push(new Coordinate(n.row, n.col+1)); } //if we aren't in the last column, add the orb to the right of this one to the stack.
            }
...
function simplify_solutions(solutions, limit) {
    ...
    var count = 1; simplified_solutions = [];
